    ...
    <>
      <main className="min-h-screen flex flex-col" style={{ background: "#0a0a0a" }}>
        ...
        <nav className="flex items-center justify-between px-8 py-6">
          ...
          >
            {["Guitars", "Keys", "Drums", "Studio"].map((item) => (
              <a key={item} href="#" className="hover:text-white transition-colors uppercase">
                {item}
              </a>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-20 text-center">
          ...
          <p
            className="mt-6 mb-12 max-w-md"
            style={{ color: "#6b6b6b", fontSize: "1.05rem", lineHeight: 1.7 }}
          >
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-5">
              {QUICK_CHIPS.map((chip) => (
                <button
                  key={chip.id}
                  type="button"
                  onClick={() => openFinder(chip.prompt)}
      ...
      {/* Full-screen Guitar Finder */}
      {finderOpen && (
        <Suspense fallback={null}>
          <GuitarFinder
            initialInput={finderInitial}
